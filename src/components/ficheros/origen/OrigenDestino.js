...
export default function OrigenDestino ({ pais, origenDestino, listDatsalDatent }) {
  ...
  return (
    <div>
      <MaquinasButtonGroup origenDestino={origenDestino} maquina={maquina} setMaquina={setMaquina} motor={motor} setMotor={setMotor} carpeta={carpeta} setCarpeta={setCarpeta} />
      <Checkbox id={todoOrigenDestino} label={'Todo'}/>
      <CampoFicheros id={'ficheros' + origenDestino} label={origenDestino + ': '} pais={ pais } maquina={maquina} motor={motor} carpeta={carpeta} listado={listDatsalDatent}/>
    </div>
  )
}
